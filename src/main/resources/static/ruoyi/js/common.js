...
function  searchFrom(dicTtable,url){
    ...
    dicTtable.reload(operParam);
}
function  searchExport(dicTtable){
    var currentId = $('form').attr('id') ;
    $("#" + currentId).attr('action', dicTtable.config.exportUrl);
    $("#" + currentId).submit();
    $("#" + currentId).attr('action', null);
}




function htmlExecCallback(regx,obj,fun){
    var val=$(obj).val();
    var index=0;
    while(true){
        var regxVal=regx.exec(val);
        if(regxVal==null||regxVal==undefined){
            return ;
        }
        fun(index,regxVal);
        index++;
